...
// restoreHostAddresses restores IP addresses in the host network namespace for a given interface.
// This is called when moving a network device back from a pod namespace to the host.
func restoreHostAddresses(ifName string, addresses []string) error {
	if len(addresses) == 0 {
		return nil
	}
	...
	for _, addrStr := range addresses {
		...
		if err != nil {
			errorList = append(errorList, fmt.Errorf("failed to parse address %s: %w", addrStr, err))
			continue
		}
		addr.Label = ifName
		if err := nhHost.AddrAdd(link, addr); err != nil && !errors.Is(err, syscall.EEXIST) {
			errorList = append(errorList, fmt.Errorf("failed to restore address %s for interface %s: %w", addrStr, ifName, err))
		}
	}
	return errors.Join(errorList...)
}
